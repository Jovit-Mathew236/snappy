...
const ScanConnector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleDisconnect = (event: USBDisconnectEvent) => {
      console.log("USB disconnect event in add-remote:", event);
      setError("USB device disconnected. Please reconnect to continue adding remotes.");
      // Note: add-remote doesn't use usbListeningRef, the loop is controlled by isListening variable
    };
  ...
  async function sendCommandAndListen(device: USBDevice) {
    let isListening = true; // Add listening control variable
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          // Remove serialNumber from localStorage
        })
      );
      ...
      // ✅ FIXED LOOP - Changed from while(true) to controlled loop
      while (isListening) {
        try {
          ...
          if (result.status === "ok") {
              ...
              if (typeof answer === "string" && answer.trim().startsWith("{")) {
                try {
                  ...
                  setStudentRemotes((prev) => {
                    if (prev.some((remote) => remote.remote_id === jsonData?.MAC)) {
                      return prev;
                    }
                    return [
                      ...prev,
            ...
            // If we get consistent errors, break the loop
          ...
          if (error instanceof DOMException &&
            (error.name === 'NetworkError' || error.name === 'NotFoundError')) {
            console.log("Device disconnected during transfer");
            isListening = false;
            break;
          }

          // For other errors, continue but with caution
          console.log("Continuing after error...");
        }
      }

      console.log("USB listening loop ended");
    } catch (error) {
        ...
        <div className="z-20">
          ...
          <div className="flex gap-10 z-20">
            <div className="flex flex-col">
              ...
              {!vendorId && (
                <button
                  className={`bg-[#5423E6] hover:bg-[#4338CA] text-nowrap rounded-2xl px-2 sm:px-4 py-2 text-[#FDFDFE] cursor-pointer font-tthoves-semiBold flex justify-center items-center align-middle ${isConnecting ? "opacity-50 cursor-not-allowed" : ""
                    }`}
